...
- **R-F-02** A `sync` command copies all supported files from the source folder into `src/content/` within the repo. `src/content/` is treated as a build artifact: fully overwritten on each sync, never hand-edited. In addition, asset files (images and PDFs) are mirrored into `public/_assets/` (flat, by basename) so the static-site build can serve them directly; `public/_assets/` is the same kind of build artifact as `src/content/` (overwritten on each sync, committed for Vercel reproducibility, never hand-edited). Filename collisions in the flat mirror emit a sync-time warning; the M4-full link index will resolve them properly.
...
- **R-F-12** Every markdown entry supports a minimum frontmatter schema: `title` (required, non-empty string), `aliases` (optional list), `summary` (optional), `portrait` (optional), plus type-specific fields defined per collection. Missing or empty `title` fails the build (R-F-13).
